...
export const errorBooks = sqliteTable(
  'error_books',
  {
    ...
    essayId: text('essay_id').references(() => essays.id, { onDelete: 'cascade' }),
  ...
  (t) => ({
    studentIdx: index('error_books_student_idx').on(t.studentId),
    essayIdx: index('error_books_essay_idx').on(t.essayId),
  }),
);
